...
if __name__ == "__main__":
    ...
    if not w3.isConnected():
        raise Exception('Connection to RPC Failed')
    ...
    if __name__ == '__main__':
        ...
        updP.start()
        pcs = [multiprocessing.Process(target=MineProcess, args=(str(minerAddress), chk, hits, bdhits, amount,
                                    amounttrigger, webhookurl, badhitbool, multibool, cudabool, False)) for x in range(0, int(intensity)*2)]
        try:
            [p.start() for p in pcs]
        except:
            raise Exception('Ram saturated')
